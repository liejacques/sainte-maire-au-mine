...
export const seedNews: News[] = articles.map((a, i) => ({
  ...
  resume: a.resume,
  contenu: a.contenu.join("\n\n"),
  image: a.image ? `(image : ${a.image.titre})` : "",
  galerie: [],
  datePublication: a.iso + "T00:00:00.000Z",
  statut: "publie",
  miseEnAvant: !!a.miseEnAvant,
}));
...
export const seedEvents: EventItem[] = evenements.map((e, i) => ({
  ...
  lieu: e.lieu,
  description: (e.contenu || [e.resume]).join("\n\n"),
  image: "",
...
export const seedUsers: User[] = [
  { ...stamp(1), nom: "Administrateur", email: "contact@sainte-marie-aux-mines.fr", role: "super_admin", actif: true },
  { ...stamp(2), nom: "Service communication", email: "communication@sainte-marie-aux-mines.fr", role: "communication", actif: true },
];
